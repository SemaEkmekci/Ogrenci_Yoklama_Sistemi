...
`);
  ...
  if (lessonResult.recordset.length === 1) {
    ...
    const result = await request.query(
      "SELECT * FROM ogrenci_ders WHERE ogrenci_id = @userID and ders_id=@lessonID"
    );
    console.log(studentInfoResult);
    console.log(result);

    const bitisSaatiString = lessonResult.recordset[0].bitis_saati
      .toISOString()
      .split("T");
    if (result.recordset.length === 1) {
      request.input(
        "studentNo",
        sql.VarChar,
        studentInfoResult.recordset[0].ogrenci_no
      );

      const attendanceCheck = await request.query(
        "SELECT * FROM yoklama_listeleri WHERE ders_id=@lessonID and ogrenci_no = @studentNo and yoklama_tarihi = @currentDate"
      );
  ...
  let hiddenStudentNo =
    studentNo.substring(0, 3) +
    "*".repeat(studentNo.length - 6) +
    studentNo.substring(studentNo.length - 2);
