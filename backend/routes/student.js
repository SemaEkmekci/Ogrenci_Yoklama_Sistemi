...
router.get("/info", async (req, res) => {
  if (req.session.no) {
    ...
    const result = await request.query(
      "SELECT ogrenci_no, ad, soyad, bolum, bolum_baslama_yili FROM ogrenci WHERE ogrenci_id = @student_no"
    );
    if (result.recordset.length === 1) {
      const { ogrenci_no, ad, soyad, bolum, bolum_baslama_yili } =
        result.recordset[0];
      res.status(200).json({
        valid: true,
        ogrenci_no,
...
`;
      ...
      if (result.recordset.length > 0) {
        ...
        res
          .status(404)
          .json({ valid: false, message: "Öğrencinin aldığı ders bulunamadı" });
      }
    } catch (error) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const result = await request.query(
      "SELECT * FROM ogrenci WHERE ogrenci_no = @studentNumber"
    );
    if (result.recordset.length === 1) {
