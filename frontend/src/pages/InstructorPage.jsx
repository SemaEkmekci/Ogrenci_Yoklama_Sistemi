    ...
    <div className="flex">
      ...
      <div className="w-full bg-gray-100 p-8">
        <div className="max-w-7xl ml-64">
          {showCards && (
            <h1 className="text-3xl font-semibold mb-8">Dersler</h1>
          )}
          <div className="flex flex-wrap">
            {showCards &&
              lessons.map((lesson, index) => (
                <div
                  key={index}
                  className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 mb-4 ml-8"
                >
                  <Card
          ...
          <div>{showActiveLesson && <ActiveLessonTable />}</div>
        </div>
      </div>
    </div>
  );
